...
/**
 * Light load test script for /api/events/week endpoint
 * Tests 10 requests/second for 10 seconds
 */
...
async function runLoadTest() {
  console.log(`🚀 Starting light load test: ${config.requestsPerSecond} req/sec for ${config.durationSeconds}s`);
  ...
  if (p95 <= maxP95 && p99 <= maxP99 && successRate >= 95) {
    console.log('✅ Light load test passed - performance within acceptable limits');
    process.exit(0);
  } else {
    console.log('❌ Light load test failed - performance issues detected');
    process.exit(1);
  }
}
